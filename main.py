...
free_tier_sessions = {}  # Store session start times for free tier users
...
bot.add_handler(MessageHandler(auth.free_tier_cmd, filters.command("free") & filters.private))
...
photologo = 'https://envs.sh/yvS.jpg' #https://envs.sh/fH.jpg/IMG20250803719.jpg
    ...
    [
        InlineKeyboardButton("『SŦɌȺNǤɆ』", url="t.me/strange_contact_robot")
            ...
            commands_list = (
                ...
                "**>  /free - ɢᴇᴛ 2 ʜᴏᴜʀꜱ ꜰʀᴇᴇ ᴜꜱᴀɢᴇ ᴘᴇʀ ᴅᴀʏ**\n"
            )
...
async def track_free_tier_usage(user_id: int, bot_username: str, seconds: int = 300):
    """
    Track free tier usage time for a user
    Default: 5 minutes (300 seconds) per command execution
    
    Args:
        user_id: Telegram user ID
        bot_username: Bot username
        seconds: Seconds of usage to add (default: 5 minutes)
    """
    try:
        ...
        # Only track if user is on free tier (not premium)
        if not has_premium and user_id != OWNER_ID and user_id not in ADMINS:
            # Check if user can still use free tier
            can_use, remaining = db.can_use_free_tier(user_id, bot_username, max_hours=2)
            if can_use:
                # Don't exceed remaining time
                seconds_to_add = min(seconds, remaining)
                if seconds_to_add > 0:
                    db.add_free_tier_usage(user_id, seconds_to_add, bot_username)
    except Exception as e:
        print(f"Error tracking free tier usage: {str(e)}")
...
async def unauthorized_handler(client, message: Message):
    ...
    try:
        ...
        if can_use:
            # User has free tier available but hasn't activated it
            remaining_hours = remaining // 3600
            remaining_mins = (remaining % 3600) // 60
            await message.reply(
                f"<b>🆓 Free Tier Available!</b>\n\n"
                f"<blockquote>You can use the bot for <b>2 hours per day</b> without admin approval.\n"
                f"You have <b>{remaining_hours}h {remaining_mins}m</b> remaining today.\n"
                f"Use <code>/free</code> to check your usage status and start using the bot.</blockquote>\n\n"
                f"<blockquote>Or contact admin to get premium access.</blockquote>",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("💫 Get Premium Access", url="t.me/strange_contact_robot")]
                ])
            )
        else:
            # User has exceeded free tier or no access
            await message.reply(
...
async def txt_handler(bot: Client, m: Message):  
    ...
    if m.chat.type == "channel":
        ...
        # Track free tier usage (5 minutes per command)
        await track_free_tier_usage(m.from_user.id, bot_username, seconds=300)
...
def reset_and_set_commands():
    ...
    {"command": "free", "description": "🆓 Get 2 hours free usage per day"},
]
